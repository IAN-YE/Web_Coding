...
var title_format = "$('input[id=\"newstitle\"]').eq(0).attr(\"value\")";
var date_format = "$('input[id=\"newsdate\"]').eq(0).attr(\"value\")";
...
function seedget() {
    request(seedURL, function (err, res, body) { //读取种子页面
        ...
        seedurl_news.each(function (i, e) { //遍历种子页面里所有的a链接
            ...
            var fetch_url_Sql = 'select url from news where url=?';
...
function newsGet(myURL) { //读取新闻页面
    request(myURL, function (err, res, body) { //读取新闻页面
        ...
        var fetchAddSql = 'INSERT INTO news (url,source_name,source_encoding,title,' +
            'keywords,author,publish_date,content) VALUES(?,?,?,?,?,?,?,?)';
        var fetchAddSql_Params = [fetch.url, fetch.source_name, fetch.source_encoding,
            fetch.title, fetch.keywords, fetch.author, fetch.publish_date, fetch.content
        ];
